#!/bin/bash
#
# RasQberry: Auto-installing IBM Quantum Courses Launcher
...
load_rqb2_env
verify_env_vars REPO USER_HOME BIN_DIR MARKER_IBM_COURSES
...
if [ ! -f "$DEMO_DIR/$MARKER_IBM_COURSES" ]; then
    info "IBM Quantum Courses not found. Installing..."
    install_demo_raspiconfig do_ibm_courses_install || die "Installation failed"
fi

# Launch the demo using the existing launcher
exec "$BIN_DIR/rq_ibm_courses.sh"
